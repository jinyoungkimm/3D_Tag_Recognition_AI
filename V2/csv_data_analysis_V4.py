...
# 푸리에 고속 변환(FFT) 알고리즘을 이용한 Convolution 연산
def convolution_average(data,kernel_size):
    ...
    # 유효한 요소의 수 계산
    valid_count = fftconvolve(mask.astype(np.float64), kernel, mode='same')

    # 마스크를 적용한 데이터 배열에 커널을 적용하여 합 계산
    summed_data = fftconvolve(masked_data, kernel, mode='same')
...
def count_cells_in_range(data, kernel_size, min_val=4, max_val=8):
    ...
    # 유효 범위 내의 셀의 개수 계산
    counted_cells = fftconvolve(combined_mask.astype(np.float64), kernel, mode='same')
